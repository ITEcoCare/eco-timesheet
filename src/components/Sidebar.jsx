...
const Sidebar = () => {
    ...
    const { activeMenu, setActiveMenu, currentColor, isHovering, handleMouseEnter, handleMouseLeave } = useStateContext();
            ...
            {activeMenu ? (<>
                <div className="flex justify-between items-center">
                    <Link to="/" onClick={() => { setActiveMenu(false) }} className="flex items-center gap-3 mt-4 ml-3 text-xl font-bold tracking-tight dark:text-white text-slate-900">
                        <ImLeaf size={30} className="" style={{color: currentColor }} />
                        <span className="text-gray-400 ">EcoTimesheet</span>
                    </Link>
                    <TooltipComponent content="Close" position="BottomCenter" className="m-2">
                        <button type="button" onClick={() => setActiveMenu((prevActiveMenu) => !prevActiveMenu)} style={{color: currentColor }} className="text-xl rounded-full p-2 mt-4 hover:bg-gray-200 hover:text-white" >
                            <CgClose />
                        </button>
                    </TooltipComponent>
                </div>
                <div className="mt-10 mb-24">
                    { menuHead.map((val) => (
                        <div key={val.title}>
                            ...
                            {val.links.map((link) => (
                                <NavLink to={`/${link.name}`} onClick={() => { }} key={link.name} 
                                    className={({ isActive }) => isActive ? activeLink : normalLink }
                                    // style={{ backgroundColor: isHovering ? 'green' : '', color: isHovering ? 'white' : '', }}
